...
<div class="relative h-full">
    <div   class="relative mx-auto max-w-7xl h-full ">
        <div class="flex h-full pb-3 gap-4 sm:gap-8 overflow-x-auto relative scroll-smooth customized-scrollbar snap-inline" bind:this={scrollContainer} >
        ...
        <button class="sm:p-4 p-3 rounded-full text-2xl  bg-gray-600  absolute top-1/2 -translate-x-8 -translate-y-1/2 sm:flex items-center shadow hidden" on:click={()=>handleScroll(-scrollAmount)}><span class="material-icons-outlined  text-center text-white">arrow_back_ios</span> </button>
        {/if}
        {#if scrollPostion < maxScrollAmount}
        <button class="sm:p-4 p-3 rounded-full text-2xl  bg-gray-600  absolute top-1/2 translate-x-8 -translate-y-1/2 right-0 sm:flex items-center shadow hidden" on:click={()=>handleScroll(scrollAmount)}><span class="material-icons-outlined  text-center text-white">arrow_forward_ios</span> </button>
        {/if}
    </div>
